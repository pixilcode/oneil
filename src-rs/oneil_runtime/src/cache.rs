...
mod python {
    ...
    use crate::{error::PythonImportError, output};
        ...
        pub fn insert(
            ...
            if let Some(matching_function_call) = matching_function_call {
                ...
                matching_function_call
                    .root_models
                    .insert(root_model.clone().into());
            } else {
                // If the function call does not exist, create a new one.
                cached_function_calls.push(FunctionCall {
                    root_models: BTreeSet::from_iter([root_model.clone().into()]),
                    inputs: args.iter().map(|arg| arg.clone().into()).collect(),
                    output: result,
                });
            }
        }
