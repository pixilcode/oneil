...
fn parse_design_bundle_decls(
    ...
    match parse_design_target_line(input) {
        Ok((rest, node)) => parse_decls_recur(rest, vec![node], vec![], false, true),
        Err(nom::Err::Error(e) | nom::Err::Failure(e)) => {
            ...
            parse_decls_recur(
                rest,
                vec![],
                vec![ParserError::design_header_not_first(span)],
                true,
                false,
            )
        }
...
fn parse_decls_recur(
    ...
    let result = parse_decl(input, allow_design_shorthand);

    match result {
        Ok((rest, decl)) => {
            acc_decls.push(decl);
            parse_decls_recur(rest, acc_decls, acc_errors, false, allow_design_shorthand)
        }
...
fn parse_section(input: InputSpan<'_>) -> Option<(InputSpan<'_>, SectionResult, SectionErrors)> {
    ...
    let (rest, decls, decl_errors) = parse_decls_recur(rest, vec![], vec![], false, false);
